<div class="grid grid-cols-[1fr] xl:grid-cols-[1fr_350px] 2xl:grid-cols-[1fr_400px] gap-5 2xl:gap-10">   
      ...
      <div class="{{ $this->checkout ? 'flex' : 'hidden' }} flex-col gap-10">
          ...
          <x-form.fieldset :title="false" set_description="Способ оплаты доставки груза" :set_loading="false">
            <div class="flex flex-col gap-4">
              <x-form.radio wire:model.live="fields.payment_method_pick" groupClass="group/radio pm-group" name="payment_method_pick" value="cash" label="Наличными при отправке" id="payment_method_pick_cash" :checked="(($this->getField('payment_method_pick') == 'cash') ? 'checked' : '')" />
              <x-form.radio wire:model.live="fields.payment_method_pick" groupClass="group/radio pm-group" name="payment_method_pick" value="bill" label="По счету" id="payment_method_pick_bill" :checked="(($this->getField('payment_method_pick') == 'bill') ? 'checked' : '')" />
            </div>
            @error('payment_method_pick')
              <span class="text-red-500 mt-4 inline-block">{{ $message }}</span>
            @enderror
                      ...
                      <div
                          ...
                          <div class="flex flex-col gap-6 w-full bg-inherit">
                              ...
                              <x-form.wrap label="Вес" name="fields.boxes_data.weight">
                                <x-form.input class="input-numeric" name="fields.boxes_data.weight" wire:model.live="fields.boxes_data.weight" />
                              </x-form.wrap>
                          </div>
                      </div>
                  </div>
                    ...
                    @if(!empty($this->getPostDate()))
                      <div class="flex items-center gap-4">
                        <p class="w-2 h-2 ml-2 ring-4 ring-amber-600"></p>
                          <p>{{ \Illuminate\Support\Carbon::parse($this->getPostDate())->translatedFormat('d F') }} отправляется с терминала</p>
                      </div>
                      <div class="">
